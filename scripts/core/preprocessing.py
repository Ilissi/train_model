...
def main(config_module):
    ...
    for trace_index, trace in enumerate(config_module.TRACES):
        ...
        with open(trace['file'], 'r') as f, open(tmpfile, 'w') as dstf:
            ...
            for line in f:
                ...
                packet.time = common_time + ((1/cf) * (packet.time - trace_time))  	#Trasladamos temporalmente el paquete
    ...
    with open(config_module.DATA_FILE, 'w') as f:
        ...
        while len(tmp_packets) > 0:
            ...
            if next_line != '': tmp_packets[next_packet] = Packet.from_csv_line(next_line)	#If not EOF: override the readed packet with the next one in that file
            else :										#else:
