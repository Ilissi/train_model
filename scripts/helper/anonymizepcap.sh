...
while getopts 'p:vh' option; do
  case $option in
    ...
    v) v=1;;
    h)
      echo "usage : anonymizepcap.sh [-v] [-p parallel_threads] srcfolder dstfolder modules"
      echo
      echo "positional arguments:"
      echo "  srcfolder             : folder to read pcaps from"
      echo "  dstfolder             : folder to save pcaps at"
      echo "  modules               : absolute path to the modules folder"
      echo
      ...
      echo
      echo "example : anonymizepcap.sh srcfolder dstfolder /absolute/path/to/modules/folder"
      echo "example : anonymizepcap.sh -p 4 -v srcfolder dstfolder /absolute/path/to/modules/folder"
      exit
  esac
done
...
if [ ! -d $modules ]; then
  echo "Modules folder does not exist"
  exit
fi
...
for filepath in $src/*.pcap; do
	...
	#Run the anonymizer
  file=$(echo $filepath | awk '{n=split($0, a, "/"); print a[n]}')
  if [ $v -eq 1 ]; then
    echo "anonymizing ${file}"
  fi
	python3 anonymizepcap.py  $src/$file $dst/$file $modules &
  pids[$empty_index]=$!
  sleep 1
